...
from tensorflow.keras.layers import Conv1D,concatenate,LSTM,Bidirectional
...
feature = np.load('onehot_m1A-101-cz-593-5930_f_train.dat')
label = np.load('onehot_m1A-101-cz-593-5930_l_train.dat')
f_test = np.load('onehot_m1A-101-cz-114-1140_f_test.dat', allow_pickle=True)
l_test= np.load('onehot_m1A-101-cz-114-1140_l_test.dat', allow_pickle=True)
...
def ANN(optimizer='adam', neurons=64,kernel_size=5, batch_size=64, epochs=48, activation='relu', patience=6,drop=0.5,
    ...
    inp1 = Input(shape=(101, 5), dtype='float32')
    ######### model1
    ...
    model = Model(inputs=inp1,outputs=final_output)  # 是keras版本的锅，Model(inputs =inp1,output = final_output)换成model = Model(inputs=..., outputs=...)
    model.summary()
    return  model
...
history = model.fit(feature, label, epochs=100, batch_size=32, verbose=2,validation_data=(f_test, l_test),
                       callbacks=[callback])
####
model.save('m1A-onehot-model.h5')
